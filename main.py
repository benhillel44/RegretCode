from system import GridSystem, System
...
def run(x_0, N_steps, _look_ahead_depth, load_memory=False, random_grid=False):
...
def test(x_0, N_steps, _look_ahead_depth, load_memory=False, random_grid=False):
    start_time = datetime.now()
    gs = GridSystem()
    gs.set_version2()
    look_ahead_policy = LookAheadPolicy(system=gs, look_ahead_depth=_look_ahead_depth, load_mem=load_memory)
